...
rushia.NewQuery("Users").Insert(rushia.H{
	"Username":  "YamiOdymel",
	"Password":  rushia.Func("SHA1(?)", "secretpassword+salt"),
	"Expires":   rushia.Now("+1Y"),
	"CreatedAt": rushia.Now(),
})
...
如果你想要在條件中宣告某個欄位是否等於某個欄位，你能夠這樣做。
...
// 等效於：SELECT * FROM Users WHERE ID NOT BETWEEN ? AND ?
```
...
// 等效於：SELECT * FROM Users WHERE ID NOT IN (?, ?, ?, ?, ?)
```
...
你亦能透過 `JoinWhere` 或 `OrJoinWhere` 擴展表格加入的限制條件。
